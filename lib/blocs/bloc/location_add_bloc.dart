...
class LocationAddBloc extends Bloc<LocationAddEvent, LocationAddState> {
  LocationAddBloc()
    ...
    on<SaveAddressEvent>((event, emit) async {
      ...
      emit(SaveAddressState(-1,
          savedLocation: savedLocation, locationList: currentList));
    });
    ...
    on<ShowSavedAddressEvent>((event, emit) async {
      ...
      emit(SaveAddressState(-1,
          savedLocation: event.locationName!, locationList: updatedList));
    });
    ...
    on<LoadAddressEvent>((event, emit) async {
      ...
      List<String> addresses = List<String>.from(
          addressBox.get('addressList', defaultValue: <String>[]));
      String savedDeliveryLocation = addressBox.get('savedAddress');
      emit(SaveAddressState(-1,
          savedLocation: savedDeliveryLocation, locationList: addresses));
    });
